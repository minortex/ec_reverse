...
结论是：本机的 `0x1B` 来自 **INT1 主机邮箱 ISR 与 `D1D1` 主线共享 register bank 2 的
寄存器重入**，再由 `D264` 写入 `0x0A54`。`1D08 -> bank1:E64F` 是重叠 wrapper 路径，
但不是 `0x1B` 的直接 producer；`0x2E/0x2F/0x31/0x32/0x37/0x38` 等周期波形属于其他
共享 scratch 活动，也不是本次选档值的来源。
