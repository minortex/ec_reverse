...
这些时间会因不满足电压或运行 gate 而延长；`XRAM[0x050B].7` 失效时则重新从零开始。
...
|---:|---:|---:|---|
| `250` | `>= 550` | `>= 0x3DE0` (15840) | GPIO 镜像值需 `>= 25` 才能单独触发，正常可达值不能满足 |
...
各行条件之间是 OR；完整的额外门槛为：250 档要求 `XRAM[0x0857] >= 25`，200 档还可由
`XRAM[0x0A54] >= 19` 或 Health 模式触发，150/100/50 档的辅助等级门槛依次是
`13/10/7`，100 档还可由 Balanced 模式触发。这里没有“battery-pack status bit 4 触发
150 档”的机器码；此前结论来自共享 helper 边界和偏移标签的误读，现已撤回。
...
找到的所有直接写入只会翻转 `0x0857`.0，所以正常可达值 0/1 始终小于 25；只有
`>= 25` 才会把降额推到 250 档。它更像复用模板留下的保护条件，不能再把它描述为已知会
触发的老化门槛。
...
| 高压热应力 | `XRAM[0x09C9:0x09CA]` | 每约 43.2 分钟增加 `0x100/0x300/0x700`，与五档阈值比较 |
| 充电模式 | `EC[0x07A6].5:4` | Balanced 至少 100 mV/cell，Health 至少 200 mV/cell |
| 模板辅助量 | `XRAM[0x0A54]`、`XRAM[0x0857]` | 本机构建的可达值分别为 1 和 0/1，不会提高档位 |
