...
- `XRAM[0x05B9] == 1` 时不执行后续档位计算；真实 `LCALL 0xD1BB` 读取该地址并在
  `0x05B9 != 1` 时返回 `R7=1`。此前把它误连到返回 `EC[0x0497].0` 的 `0xE56F`，是
  重叠函数边界错误；`0x0497` 被其他电池路径使用，但不在这条老化调用链上；
...
因此在电压条件成立时，大约每累计 43.2 分钟才向应力 word 增加一次温度权重。低于电压
...
`CODE:0xD1BB` 检查 `XRAM[0x05B9]`；值等于 1 时返回 `R7=0` 并使函数退出，值不等于 1
时明确返回 `R7=1`。随后 `EC[0x0490].2` 条件成立才调用 `CODE:0x1D08`，返回后未经变换
便执行：
...
单字节 `RET`，全程没有改写 `R7`。因此只要这次调用执行到该写入及后续降额决策，写入
`XRAM[0x0A54]` 的局部到达值就确定为 1。这不是对主机不可见 XRAM 当前值的实测，也不
表示该共享 scratch 在其他函数执行期间恒为 1。
...
if (XRAM[0x05B9] == 1 || !power_source_bit2)
    return;
...
| stress 的形成 | 包电压是否达到 `4100 mV/cell`、温度、约 43.2 分钟窗口 | 电压决定是否累计，温度决定每次 `+1/+3/+7`；不会即时直接减 target |
| 更新门控 | `ApExistFlag`、`0x0490` bit 1/2/0、`0x05B9` | 决定清模式、清 stress、提前返回或是否写回；关闭时可能保留旧 target |
