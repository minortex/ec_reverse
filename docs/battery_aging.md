...
- `XRAM[0x050B].7` 为 0 时，`XRAM[0x09C7:0x09CA]` 全部清零并立即返回；机器码会对
  `XRAM[0x050A:0x050B]` 的负值取 16 位补码绝对值并发布到公开 discharge-current 字段，
  同步采样也与 mA 数值吻合，因此它是 signed battery current：正值充电、负值放电；
- `EC[0x0497].0` 为 0 时，不执行后续档位计算；该位在电池缓存同步及初始化后与瞬态 bit 7
  一起置位，状态机重启时清零，并被多条电池数据路径共享，因此中等置信度解释为
  `battery_data_session_initialized`，不是老化算法专用开关；
...
窗口不会增加应力。signed battery current 的负号 gate `XRAM[0x050B].7` 失效时，函数会把
`0x09C7–0x09CA` 全部清零，所以它是当前运行状态内的高压热应力，不是写入 Flash 的
终身累计量。多轮离电/插电时间线显示离电高字节为 `0xF9–0xFD`，充电为 `0x00–0x07`，
已验证该 gate 只在电池对系统放电时开放。
...
恢复或持久化生产者：写入只来自 `D1D1` 的四字节清零、两级 60 次计数，以及最终
`+1/+3/+7` helper。相邻 `.d52` 符号受 `org -1` 影响，其他看似命中的 `dptr_09CA`
实际访问 `0x09CB`，不能算作 stress 写入。当前镜像也没有从 Flash、EEPROM 或电池缓存把
stress 灌回来的直接路径。

这使 stress 档在本机配置下接近不可达：它只在 signed battery current 为负时累计，而一次
充电、零电流或插电过渡就清零。即使始终处于最高温度 `+7`，最低 50 mV/cell 档也需要
约 18.54 天连续满足“放电且包电压不低于 4.10 V/cell”；真实电池无法在该电压区间连续
放电这么久。因此继续探索这条 score 路径的最高价值问题，不是阈值本身，而是比较其他
项目/固件版本是否翻转了 current gate、缩短了计数周期，或给 stress 增加了持久化恢复。
在当前 2.12 镜像中，不能把 stress 当作产生现有 250 mV/cell 差值的现实来源。

### current gate 正负号复核

这里的反常极性不是 Ghidra 或 `.d52` 把条件翻译反了。原始机器码链为：

```text
E6FE  MOV  DPTR,#0x050B
E701  MOVX A,@DPTR
E702  ANL  A,#0x80
E704  RET

D1E3  LCALL E6FE
D1E6  JNZ   D1F7       ; bit 7 == 1，即 signed word < 0，才继续
D1E8  ...              ; bit 7 == 0 时清零并 RET
```

8051 的 `JNZ` 测试 A 是否非零；这里没有 carry、有符号比较或反编译器条件恢复的歧义。
真实地址也由原始立即数 `0x050B` 确认，不受 `.d52 org -1` 影响。另一方面，发布公开电流的
`0x2CF3–0x2D3A` 会在同一 high byte 的 bit 7 成立时取 16 位补码绝对值，证明 bit 7 的确是
该 word 的负号，而不是独立状态 flag。

方向语义来自多轮同步实测：离电时 high byte 为 `0xF9–0xFD`，插电充电时为
`0x00–0x07`。所以当前项目运行时是“正充、负放”，而 `D1D1` 的确只让负值通过。尚不能
仅凭本镜像断言源代码作者把一条指令写错；同样符合证据的解释还有：

1. 通用模板原本期待相反的电流极性，本项目遥测发布端采用了正充负放；
2. 这段 stress 累计在本项目配置中本来就是不可达的保留模板；
3. `JNZ`/清零分支确实是厂商逻辑错误。

要区分三者，应比较同系列其他项目/版本在对应位置的原始字节和电流发布约定。当前仓库只
有这一份相同 blob 的主 bank1 副本，无法做独立版本投票。无论源代码意图是哪一种，本机
2.12 的实际执行效果已经确定：充电清零，放电才可能累计。
...
if (!battery_data_session_initialized || !power_source_bit2)
    return;
